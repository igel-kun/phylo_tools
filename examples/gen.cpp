...
void get_node_numbers(long& num_nodes, long& num_retis, long& num_leaves)
{
  ...
  } catch(const std::invalid_argument& err){ cfail << "problem converting argument to integer: "<<err.what()<<std::endl;
  } catch(const std::logic_error& err){ cfail << "cannot generate such a network: "<<err.what()<<std::endl; }
}
