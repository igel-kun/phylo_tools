...
#define ISOLATE_HIGHEST_BIT(x) (uint64_t{1} << (63 - NUM_LEADING_ZEROSLL(x)))
...
[[noreturn]] void cfail(const auto& t) {
  std::cerr << t;
  exit(EXIT_FAILURE);
};
