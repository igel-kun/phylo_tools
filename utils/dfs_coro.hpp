...
resume_ascending:
            ...
            switch(may_not_visit_next()) {
              ...
              case 1:
                  if constexpr (is_all_edge_traversal(tt)) return 4; // resume at &&resume_ascending
              default: break;
            }
            children.pop_back();
//          }
...
inner_loop_end:
//        }
        if constexpr (is_node_traversal(tt) && is_postorder_traversal(tt)) return 5; // resume at &&resume_outer
resume_outer:
