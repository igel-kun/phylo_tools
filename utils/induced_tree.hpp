...
namespace PT{
  ...
  template<class T> 
  concept SubtreeInfo = requires(T a) {
    requires std::derived_from<T, SparseInducedSubtreeInfoMap>;
  };
  template<class M>
  concept SubtreeInfoMap = requires(M m) {
    requires std::MapType<M>;
    requires SubtreeInfo<typename M::mapped_type>;
  };



  // compute necessary information from a tree
  template<PhylogenyType Tree, SubtreeInfoMap NodeInfoMap = InducedSubtreeInfoMap>
  ...
  template<PhylogenyType Tree, SubtreeInfoMap NodeInfoMap>
  ...
  template<PhylogenyType Tree, NodeContainerType LeafList, SubtreeInfoMap NodeInfoMap>
  ...
  protected:
    inline void append_unless_equal(EdgeVec& ev, const NodeDesc u, const NodeDesc v) { if(u != v) append(ev, u, v); }
    ...
    void compute_inner_nodes()
    {
      ...
      for(auto iter = leaves_sorted->begin(); iter != leaves_sorted->end();){
        ...
        const NodeDesc l = supertree.LCA(u, *iter);
  ...
  template<PhylogenyType Tree, NodeContainerType LeafList, SubtreeInfoMap NodeInfoMap = SparseInducedSubtreeInfoMap>
  auto get_induced_edges(const policy_noop_t, const Tree& supertree, const LeafList& leaves, std::shared_ptr<NodeInfoMap> node_infos = {})
  {
  ...
  template<PhylogenyType Tree, NodeContainerType LeafList, SubtreeInfoMap NodeInfoMap = InducedSubtreeInfoMap>
  auto get_induced_edges(const policy_copy_t, const Tree& supertree, const LeafList& leaves, std::shared_ptr<NodeInfoMap> node_infos = {})
  {
    ...
    return get_induced_edges(policy_inplace_t(), supertree, leaves_local, std::move(node_infos));
  }
  ...
  template<PhylogenyType Tree, NodeContainerType LeafList, SubtreeInfoMap NodeInfoMap = InducedSubtreeInfoMap> requires (!std::is_const_ref<LeafList>)
  auto get_induced_edges(const policy_inplace_t, const Tree& supertree, LeafList&& leaves, std::shared_ptr<NodeInfoMap> node_infos = {})
  {
    make_node_infos(supertree, node_infos);
  ...
  template<PhylogenyType Tree, NodeContainerType LeafList, SubtreeInfoMap NodeInfoMap = SparseInducedSubtreeInfoMap>
  auto get_induced_edges(const Tree& supertree, LeafList&& leaves, std::shared_ptr<NodeInfoMap> node_infos = {})
  {
    if constexpr (std::is_const_ref<LeafList>)
      return get_induced_edges(policy_copy_t(), supertree, leaves, std::move(node_infos));
    else
      return get_induced_edges(policy_inplace_t(), supertree, leaves, std::move(node_infos));
  }
