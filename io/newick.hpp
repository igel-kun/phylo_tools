...
namespace PT{
  ...
  std::string get_extended_newick(const _Network& N, const uint32_t sub_root, std::iterable_bitset& retis_seen)
  {
    const typename _Network::Node& v = N[sub_root];
    std::string accu = "";
  ...
  //        throw MalformedNewick(back, "reticulations may not have multiple children");
    ...
    inline void put_branch_in_edgelist(EdgeList& el, const Edge& e, const float& len) const
    {
      el.emplace_back(e);
    }
    inline void put_branch_in_edgelist(EdgeVec& el, const Edge& e, const float& len) const
    {
      el.emplace_back(e);
    }
    inline void put_branch_in_edgelist(WEdgeList& el, const Edge& e, const float& len) const
    {
      el.emplace_back(e, len);
    }
    inline void put_branch_in_edgelist(WEdgeVec& el, const Edge& e, const float& len) const
    {
      el.emplace_back(e, len);
    }
    ...
    float read_length()
    {
      ...
      if((sep != std::string::npos) && (s.at(sep) == ':')){
        const float result = std::strtof(s.c_str() + sep + 1, NULL);
        back = sep - 1;
        return result;
      } else return 0;
    }
